<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Editar Jornada') }} - GW{{ $gameweek->number }}</h2>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 rounded-xl shadow">
                <form method="POST" action="{{ route('admin.gameweeks.update', [app()->getLocale(), $gameweek]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="season_id" 
                            ...
                            @foreach($seasons as $season)
                                <option value="{{ $season->id }}" 
                                        {{ old('season_id', $gameweek->season_id) == $season->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" 
                               id="number" 
                               name="number" 
                               value="{{ old('number', $gameweek->number) }}"
                               placeholder="1"
                               min="1"
                               max="30"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" 
                               id="starts_at" 
                               name="starts_at" 
                               value="{{ old('starts_at', $gameweek->starts_at->format('Y-m-d\TH:i')) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" 
                               id="ends_at" 
                               name="ends_at" 
                               value="{{ old('ends_at', $gameweek->ends_at->format('Y-m-d\TH:i')) }}"
                    ...
                    <div class="mb-4">
                        <label class="flex items-center">
                            <input type="checkbox" 
                                   name="is_playoff" 
                                   value="1"
                                   {{ old('is_playoff', $gameweek->is_playoff) ? 'checked' : '' }}
                    ...
                    <div id="playoff_round_section" class="mb-4 {{ old('is_playoff', $gameweek->is_playoff) ? '' : 'hidden' }}">
                        ...
                        <select id="playoff_round" 
                            ...
                            <option value="1" {{ old('playoff_round', $gameweek->playoff_round) == 1 ? 'selected' : '' }}>{{ __('Cuartos de Final') }}</option>
                            <option value="2" {{ old('playoff_round', $gameweek->playoff_round) == 2 ? 'selected' : '' }}>{{ __('Semifinales') }}</option>
                            <option value="3" {{ old('playoff_round', $gameweek->playoff_round) == 3 ? 'selected' : '' }}>{{ __('Final') }}</option>
                        </select>
                        @error('playoff_round')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            <input type="checkbox" 
                                   name="is_closed" 
                                   value="1"
                                   {{ old('is_closed', $gameweek->is_closed) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                            <span class="ml-2 text-sm text-gray-700">{{ __('Jornada cerrada') }}</span>
                        </label>
