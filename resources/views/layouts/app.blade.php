        ...
        >
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                class="flex items-center px-3 py-2.5 rounded-lg transition group {{ request()->routeIs('manager.dashboard') ? 
                    ...
                    <span x-show="sidebarOpen" x-transition class="ml-3 font-medium">{{ __('Home') }}</span>
                </a>
                ...
                {{-- Clasificación --}}
                @if(auth()->user()->hasRole('manager'))
                    <a href="{{ route('manager.league.standings', ['locale' => app()->getLocale()]) }}" 
                    class="flex items-center px-3 py-2.5 rounded-lg {{ request()->routeIs('manager.league.*') ? 
                        'bg-emerald-500/20 text-emerald-400' : 'text-gray-300 hover:bg-white/5 hover:text-white' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Divider --}}
                <div x-show="sidebarOpen" x-transition class="pt-4 pb-2">
                    <div class="px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('Edu Can') }}</div>
                </div>

                {{-- EDUCATION (Módulo Educativo) --}}
                <a href="{{ route('manager.education.index', ['locale' => app()->getLocale()]) }}" 
                class="flex items-center px-3 py-2.5 rounded-lg transition group {{ request()->routeIs('manager.education.*') 
                    ...
                    <svg class="w-5 h-5 flex-shrink-0 transition-transform group-hover:scale-110" 
                        :class="sidebarOpen ? 'mr-3' : 'mx-auto'" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
                    </svg>
                    <span x-show="sidebarOpen" x-transition class="font-medium">{{ __('Education') }}</span>
                    @if(request()->routeIs('manager.education.*'))
                ...
                {{-- Configuración --}}
                @if(auth()->user()->hasRole('manager'))
                    <a href="{{ route('manager.settings.index', ['locale' => app()->getLocale()]) }}" 
                    class="flex items-center px-3 py-2.5 rounded-lg {{ request()->routeIs('manager.settings.*') ? 
                        'bg-emerald-500/20 text-emerald-400' : 'text-gray-300 hover:bg-white/5 hover:text-white' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
