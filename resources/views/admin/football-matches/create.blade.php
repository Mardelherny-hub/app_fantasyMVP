<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Crear Partido') }}</h2>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white p-6 rounded-xl shadow">
                <form method="POST" action="{{ route('admin.football-matches.store', app()->getLocale()) }}">
                    @csrf
                    ...
                    {{-- Estado y Marcador (opcional) --}}
                    <div class="mb-4 p-4 bg-blue-50 rounded-lg">
                        <h3 class="text-sm font-semibold text-gray-700 mb-3">{{ __('Estado y Resultado (Opcional)') }}</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select id="status" 
                                    ...
                                    <option value="0" {{ old('status', '0') == '0' ? 'selected' : '' }}>{{ __('Pendiente') }}</option>
                                    <option value="1" {{ old('status') == '1' ? 'selected' : '' }}>{{ __('En Vivo') }}</option>
                                    <option value="2" {{ old('status') == '2' ? 'selected' : '' }}>{{ __('Finalizado') }}</option>
                                    <option value="3" {{ old('status') == '3' ? 'selected' : '' }}>{{ __('Pospuesto') }}</option>
                                </select>
                            </div>

                            {{-- Goles (solo si no es pendiente) --}}
                            <div id="goals_section" class="col-span-2 grid grid-cols-2 gap-4 {{ old('status', '0') == '0' ? 'hidden' : '' }}">
                                <div>
